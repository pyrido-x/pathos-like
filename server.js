...
// ═══════ 常量 ═══════
const VOID=0,WALL=1,FLOOR=2,CLOSED_DOOR=3,OPEN_DOOR=4,UP=5,DOWN=6;
const MW=80,MH=60,S=11,MCD=200,RSP=4000,MAX_DEPTH=50;
...
function genDungeon(lv){
  ...
  for(let gy=0;gy<3;gy++){
    for(let gx=0;gx<3;gx++){
      const zx=gx*GW, zy=gy*GH, mg=3;
      const zw=GW-mg*2, zh=GH-mg*2;
      const rw=ri(7,Math.min(16,zw-2)), rh=ri(5,Math.min(12,zh-2));
      const rx=zx+mg+ri(0,Math.max(0,zw-rw)), ry=zy+mg+ri(0,Math.max(0,zh-rh));
      for(let y=ry-1;y<=ry+rh;y++)for(let x=rx-1;x<=rx+rw;x++)if(y>=0&&y<MH&&x>=0&&x<MW)T[y][x]=WALL;
      for(let y=ry;y<ry+rh;y++)for(let x=rx;x<rx+rw;x++)T[y][x]=FLOOR;
      const ri2=rooms.length;
      rooms.push({x:rx,y:ry,w:rw,h:rh,cx:Math.floor(rx+rw/2),cy:Math.floor(ry+rh/2),idx:ri2});

      // 墙上开1~3个门（初始关闭）
      const wc=[];
      for(let x=rx+1;x<rx+rw-1;x++){wc.push({x,y:ry-1});wc.push({x,y:ry+rh});}
      for(let y=ry+1;y<ry+rh-1;y++){wc.push({x:rx-1,y});wc.push({x:rx+rw,y});}
      const v=wc.filter(w=>w.x>0&&w.x<MW-1&&w.y>0&&w.y<MH-1);
      if(!v.length)continue;
      const nD=Math.min(ri(1,3),v.length), ch=new Set();
      for(let i=0;i<nD;i++)ch.add(ri(0,v.length-1));
      for(const ci of ch){const w=v[ci];T[w.y][w.x]=CLOSED_DOOR;allDoors.push({x:w.x,y:w.y,roomIdx:ri2});}
    }
  }

  // ── 每房间的门 ──
  const rDoors=Array.from({length:rooms.length},()=>[]);
  for(const d of allDoors) rDoors[d.roomIdx].push(d);

  // ── MST 连通 + 走廊路径 ──
  const edges=[];
  for(let i=0;i<rooms.length;i++){if(!rDoors[i].length)continue;for(let j=i+1;j<rooms.length;j++){if(!rDoors[j].length)continue;for(const da of rDoors[i])for(const db of rDoors[j])edges.push({a:i,b:j,da,db,w:dist(da.x,da.y,db.x,db.y)});}}
  ...
  const mst=[];for(const e of edges)if(uni(e.a,e.b))mst.push(e);
  ...
  for(const conn of mst){
    ...
    const path=[];
    if(Math.random()<0.5){while(cx!==tx){cx+=cx<tx?1:-1;path.push({x:cx,y:cy});}while(cy!==ty){cy+=cy<ty?1:-1;path.push({x:cx,y:cy});}}else{while(cy!==ty){cy+=cy<ty?1:-1;path.push({x:cx,y:cy});}while(cx!==tx){cx+=cx<tx?1:-1;path.push({x:cx,y:cy});}}
    for(const p of path)if(p.x>=0&&p.x<MW&&p.y>=0&&p.y<MH)T[p.y][p.x]=FLOOR;
    corridors.push({path,da,db});
  }

  // ── doorMap: 每扇门通过走廊连接的另一扇门 + 走廊路径 ──
  const doorMap={};
  for(const d of allDoors) doorMap[`${d.x},${d.y}`]=[];
  for(const c of corridors){
    const ka=`${c.da.x},${c.da.y}`, kb=`${c.db.x},${c.db.y}`;
    if(doorMap[ka]) doorMap[ka].push({door:c.db,path:c.path});
    if(doorMap[kb]) doorMap[kb].push({door:c.da,path:c.path});
  }

  // ── 楼梯：上楼和下楼在不同随机房间 ──
  const roomIdx=Array.from({length:rooms.length},(_,i)=>i);
  const riUp=rp(roomIdx);
  const riDn=rp(roomIdx.filter(i=>i!==riUp)); // 不和上楼同一房间
  T[rooms[riUp].cy][rooms[riUp].cx]=UP;
  T[rooms[riDn].cy][rooms[riDn].cx]=DOWN;

  // ── 怪物 ──
  const mons=[];
  const pool=spawnP(lv);
  for(let i=0;i<ri(4,9)+lv*2;i++){
  ...
  if(lv>=3&&Math.random()<0.12){const rm=rp(rooms.slice(-3)),nm=M.nemesis;mons.push({id:uid(),key:'nemesis',x:rm.cx,y:rm.cy,lv,n:nm.n,e:nm.e,hp:nm.hp+ri(0,25),mhp:nm.hp+25,st:nm.st,dx:nm.dx,xp:nm.xp,la:0});}
  ...
  return {tiles:T,rooms,monsters:mons,items,lvl:lv,doors:allDoors,doorMap,corridors,openDoors,upRoom:rooms[riUp],downRoom:rooms[riDn]};
}
...
class Game {
  ...
  openDoor(lv, dx, dy, seen){
    ...
    for(const {door,path} of linked){
      ...
      // 揭示对面那扇门（但它还是关着的，玩家看不到门后房间）
      seen.add(`${door.x},${door.y}`);
      // ← 这里不揭露对面房间！等玩家走过去推开对面的门再说
    }
  ...
  enterRoom(lv, x, y, seen){
    const ri2=this.findRoom(lv,x,y);
    if(ri2>=0) this.revealRoom(lv,ri2,seen);
    // 也揭示周围的开放门
    for(const d of lv.doors){
      if(dist(x,y,d.x,d.y)<=1) seen.add(`${d.x},${d.y}`);
    }
  }
  ...
  revealRoom(lv,ri2,seen){
    ...
    for(let y=r.y-1;y<=r.y+r.h;y++)
      for(let x=r.x-1;x<=r.x+r.w;x++)
        if(x>=0&&x<MW&&y>=0&&y<MH&&lv.tiles[y][x]!==VOID)
          seen.add(`${x},${y}`);
  }
  ...
  findRoom(lv,x,y){
    for(let i=0;i<lv.rooms.length;i++){
      const r=lv.rooms[i];
      // 房间内部 + 墙壁
      if(x>=r.x-1&&x<=r.x+r.w&&y>=r.y-1&&y<=r.y+r.h) return i;
    }
    return -1;
  }
  ...
  // ─── 玩家 ───
  addPlayer(sock,d){
    const nm=(d.name||'冒险者').slice(0,12);
    const cl=CLS[d.class]?d.class:'warrior';
    const t=CLS[cl], lv=this.gl(1);
    // 出生在上楼楼梯旁边
    ...
    sock.emit('welcome',{you:this.san(p,true),map:lv.tiles,level:1,openDoors:[...lv.openDoors]});
  ...
  et(sid){
    ...
    const lv=this.gl(p.level);if(!lv||!lv.monsters)return;
    const now=Date.now();
    for(const m of lv.monsters){
      if(m.hp<=0)continue;if(now-(m.la||0)<450)continue;
      const d=dist(m.x,m.y,p.x,p.y);if(d>S+2)continue;
      if(d<=1){this.mh(m,p);m.la=now;}else if(d<=S+2)this.ch(m,p,lv,now);
    }
    ...
    if(!p.alive&&p.dt&&now-p.dt>RSP)this.rp(p);
    this.bc(p.level);
  }

  ch(m,tg,lv,now){
    const dx=Math.sign(tg.x-m.x),dy=Math.sign(tg.y-m.y);
    const mv=Math.abs(tg.x-m.x)>Math.abs(tg.y-m.y)?[{x:dx,y:0},{x:0,y:dy}]:[{x:0,y:dy},{x:dx,y:0}];
    for(const s of mv){const nx=m.x+s.x,ny=m.y+s.y;if(tg.x===nx&&tg.y===ny){this.mh(m,tg);m.la=now;return;}if(this.cs(nx,ny,lv)){m.x=nx;m.y=ny;m.la=now;return;}}
  }
  cs(x,y,lv){
    if(x<0||x>=MW||y<0||y>=MH)return false;
    if(lv.tiles[y][x]===VOID||lv.tiles[y][x]===WALL)return false;
    if(lv.tiles[y][x]===CLOSED_DOOR)return false; // 怪物不能开闭着的门
  ...
  mv(sid,dx,dy){
    const p=this.players.get(sid);if(!p||!p.alive)return;
    if(Date.now()-p.lm<MCD)return;p.lm=Date.now();
    const nx=p.x+dx,ny=p.y+dy;
    const lv=this.gl(p.level);if(!lv)return;
    ...
    if(lv.tiles[ny][nx]===CLOSED_DOOR){
      // 开门揭露另一侧
      this.openDoor(lv, nx, ny, p.seenTiles);
      // 标记为打开
      lv.tiles[ny][nx]=OPEN_DOOR;
      // 穿过门进入走廊/房间
      p.x=nx;p.y=ny;
      // 如果门后是房间，揭露整个房间
      this.enterRoom(lv,nx,ny,p.seenTiles);
      this.lg(sid,'🚪 你推开了门。');
      this.ss(this.socks.get(sid),p.level);
      this.act(sid);return;
    }
    ...
    if(lv.tiles[ny][nx]===WALL){
      ...
      if(mon)this.ph(p,mon,lv);this.act(sid);return;
    }
  ...
  cl(sid,dir){
    const p=this.players.get(sid);if(!p||!p.alive)return;
    const lv=this.gl(p.level);const tile=lv.tiles[p.y][p.x];

    if(dir==='down'&&tile===DOWN){
      if(p.level>=MAX_DEPTH){this.lg(sid,'⚠️ 这已经是地牢最深处，无法继续下行。');return;}
      p.level++;p.depth=Math.max(p.depth,p.level);
      const nl=this.gl(p.level);
      // 下楼后出生在上楼楼梯旁
      p.x=nl.upRoom.cx;p.y=nl.upRoom.cy;
      p.seenTiles=new Set();this.enterRoom(nl,p.x,p.y,p.seenTiles);
      this.lg(sid,`⬇️ 下到第${p.level}层 / 最深${MAX_DEPTH}层`);
      this.bc(p.level-1);this.socks.get(sid)?.emit('newLevel',{map:nl.tiles,level:p.level,openDoors:[...nl.openDoors]});
      this.bc(p.level);this.la(`⬇️ ${p.name}下到第${p.level}层。`);
    }else if(dir==='up'&&tile===UP){
      if(p.level===1){
        // ── 从第1层上楼 = 逃离地牢！ ──
        p.escaped=true;p.alive=false;
        const score=this.calcScore(p);
        this.lg(sid,`🏆 恭喜！你成功逃出了地牢！`);
        this.lg(sid,`📊 得分: ${score} (等级${p.lvl} × 击杀${p.kills} × 金币${p.gold} × 最深${p.depth}层)`);
        this.la(`🏆 ${p.name} 逃出了地牢！得分: ${score}`);
        // 清理当前层
        this.bc(1);
        // 发送逃离事件
        this.socks.get(sid)?.emit('escaped',{score,name:p.name,lvl:p.lvl,kills:p.kills,gold:p.gold,depth:p.depth});
        return;
      }
      p.level--;
      const nl=this.gl(p.level);
      // 上楼后出生在下楼楼梯旁
      p.x=nl.downRoom.cx;p.y=nl.downRoom.cy;
      p.seenTiles=new Set();this.enterRoom(nl,p.x,p.y,p.seenTiles);
      this.lg(sid,`⬆️ 上到第${p.level}层`);
      this.bc(p.level+1);this.socks.get(sid)?.emit('newLevel',{map:nl.tiles,level:p.level,openDoors:[...nl.openDoors]});
      this.bc(p.level);this.la(`⬆️ ${p.name}上到第${p.level}层。`);
    }else{this.lg(sid,'这里没有楼梯。');return;}
    this.act(sid);
  }

  calcScore(p){
    return (p.lvl*100) + (p.kills*50) + (p.gold) + (p.depth*200);
  }
...
srv.listen(PORT,'0.0.0.0',()=>console.log(`🏰 Pathos 开门迷雾 v7 :${PORT}`));
