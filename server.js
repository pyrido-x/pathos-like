...
const MOVE_CD = 180;           // 玩家行动冷却 (ms)
const SIGHT  = 8;              // 怪物视野
const RESPAWN_MS = 4000;       // 死亡后复活时间
...
function genDungeon(lvl) {
  ...
  for(let i=0;i<nMon;i++){
    ...
    monsters.push({ id:uid(), key:rp(pool), x:ri(rm.x,rm.x+rm.w-1), y:ri(rm.y,rm.y+rm.h-1), lvl,
      hp:0, maxHp:0, st:0, dx:0, xp:0, n:'', e:'', lastAct:0 });
  }
...
class Game {
  ...
  enemyTurn(sid) {
    ...
    // 再生戒指
    if (p.ring && p.ring.ring === 'regen' && p.hp < p.maxHp) {
  ...
  chaseMonster(mon, target, lv, now) {
    ...
    for (const m of moves) {
      ...
      if (target.x === nx && target.y === ny) {
        this.monsterHits(mon, target);
        mon.lastAct = now;
        return;
      }
      if (this.canStand(nx, ny, lv)) {
        mon.x = nx; mon.y = ny;
        mon.lastAct = now;
        return;
      }
  ...
  movePlayer(sid, dx, dy) {
    ...
    if (here.length) {
      this.log(sid, `📦 地上有：${here.map(i=>i.n).join('、')}。按 G 拾取。`);
    }
      ...
      if (fx) {
        for (const f of fx) {
          ...
          switch(f.t){
            ...
            case 'resist':
              if(f.res==='全') Object.keys(p.resists).forEach(k=>p.resists[k]=true);
  ...
  chat(sid, msg) {
    ...
    const text = msg.trim().slice(0,150);
  ...
  heartbeat() {
    ...
    for (const p of this.players.values()) {
      if (p.alive) activeLevels.add(p.level);
    }
    for (const lvl of activeLevels) {
      this.broadcastLevel(lvl);
    }
  }
}
...
io.on('connection', socket => {
  ...
  socket.on('move', data => game.movePlayer(socket.id, data.dx||0, data.dy||0));
...
// 心跳：每秒检查复活和广播
setInterval(() => game.heartbeat(), 1000);

const PORT = process.env.PORT || 3000;
server.listen(PORT, () => {
  console.log('═══════════════════════════════════════');
  console.log('  🏰 Pathos 在线 — 多人回合制地牢');
